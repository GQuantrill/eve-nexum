...
- **Auto-admit friends by standing (optional, off by default)** — instead of adding corps one by one, you can flip a switch so that anyone your corp/alliance holds at a chosen in-game standing (**+10 only**, or **+5 and above**) may log in automatically. On an alliance installation this reads the alliance's contacts; on a corp installation it reads the corp's. Only positive standings qualify (neutral and negative never do), and it needs your contacts synced first (a director-role character has to sync them via the **Sync standings** button). Turning it off reverts to the explicit allow-list.
